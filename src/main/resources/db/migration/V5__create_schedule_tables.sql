...
CREATE TABLE `schedule_generation_jobs` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT 'AI 일정 생성 작업 ID',
	...
	CONSTRAINT `PK_SCHEDULE_GENERATION_JOBS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `schedule_generation_snapshots` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '생성 입력 설문 스냅샷 ID',
	...
	`created_at`	DATETIME(6)	NOT NULL	DEFAULT CURRENT_TIMESTAMP(6),
	CONSTRAINT `PK_SCHEDULE_GENERATION_SNAPSHOTS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `schedules` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '일정 버전 ID',
	`trip_id`	BIGINT	NOT NULL	COMMENT '여행방 ID',
...
CREATE TABLE `schedule_days` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '일정 일차 ID',
	...
	CONSTRAINT `PK_SCHEDULE_DAYS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `schedule_visits` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '일정 방문 장소 ID',
	...
	CONSTRAINT `PK_SCHEDULE_VISITS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `schedule_legs` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '장소 간 이동 구간 ID',
	`schedule_day_id`	BIGINT	NOT NULL	COMMENT '일정 일차 ID',
	...
	CONSTRAINT `PK_SCHEDULE_LEGS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `weather_checks` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '여행 D-1 날씨 조회 ID',
	`trip_id`	BIGINT	NOT NULL	COMMENT '여행방 ID',
	...
	CONSTRAINT `PK_WEATHER_CHECKS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `weather_schedule_decisions` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '날씨 기반 일정 결정 ID',
	...
	CONSTRAINT `PK_WEATHER_SCHEDULE_DECISIONS` PRIMARY KEY (
		`id`
	)
);
