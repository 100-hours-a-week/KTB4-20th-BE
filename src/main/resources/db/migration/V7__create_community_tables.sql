...
CREATE TABLE `community_posts` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '커뮤니티 일정 게시물 ID',
	`like_count`	INT	NOT NULL	DEFAULT 0,
	`view_count`	INT	NOT NULL	DEFAULT 0,
	`popularity_score`	DECIMAL(20, 8)	NOT NULL	DEFAULT 0	COMMENT '시간 경과를 반영한 인기 점수',
	`created_at`	DATETIME(6)	NOT NULL	DEFAULT CURRENT_TIMESTAMP(6),
	CONSTRAINT `PK_COMMUNITY_POSTS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `trip_course_community_posts` (
	...
	CONSTRAINT `PK_TRIP_COURSE_COMMUNITY_POSTS` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `community_likes` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '커뮤니티 좋아요 ID',
	`community_post_id`	BIGINT	NOT NULL	COMMENT '게시물 ID',
	`user_id`	BIGINT	NOT NULL	COMMENT '좋아요 사용자 ID',
	CONSTRAINT `PK_COMMUNITY_LIKES` PRIMARY KEY (
		`id`
	)
);
...
CREATE TABLE `community_view_events` (
	`id`	BIGINT	NOT NULL AUTO_INCREMENT	COMMENT '게시물 조회 이벤트 ID',
	`community_post_id`	BIGINT	NOT NULL	COMMENT '게시물 ID',
	`user_id`	BIGINT	NULL	COMMENT '로그인 사용자 ID',
	CONSTRAINT `PK_COMMUNITY_VIEW_EVENTS` PRIMARY KEY (
		`id`
	)
);
